...
const textClasses = {
  highlight: 'font-bold text-sm uppercase tracking-widest whitespace-nowrap bg-clip-text text-transparent bg-gradient-to-r mx-5 from-purple-400 to-pink-400',
  solid: 'font-bold text-sm uppercase tracking-widest whitespace-nowrap text-neutral-600 dark:text-neutral-400 group-hover:text-black dark:group-hover:text-white transition-colors duration-300',
              ...
              {texts.map((item, index) => (
                <div key={`${i}-${index}`} className="flex items-center gap-2">
                  <span
                    ...
                    style={item.type === 'outline' ? { WebkitTextStroke: '1px #525252' } : undefined}
                  >
                    {item.text}
                  </span>
