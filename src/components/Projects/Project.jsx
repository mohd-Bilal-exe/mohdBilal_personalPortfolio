...
export default function Project({ isFade, setCursorColor }) {
  const handleEnter = () => {
    setCursorColor({
      color: 'bg-gradient-to-br from-cyan-500 to-green-600 shadow-green-600',
      size: 'w-4 h-4',
    });
  };
  const handleExit = () => {
  ...
  return (
    ...
    >
      ...
      <ProjectGridLayout
        key="Projects"
        layouts={{
          lg: projectLayouts.lg,
          md: projectLayouts.md,
          sm: projectLayouts.sm,
          xxs: projectLayouts.xs,
        }}
              ...
              >
                ...
                <div className="space-y-2 p-4">
                  <div className="flex items-center gap-3">
                    <h1 className="font-semibold smartphone:text-lg text-xl tracking-wider transition-all group-hover:translate-x-4 duration-700 ease-in-out Pally">
                      {Project.title}
                    </h1>

                    {/* 👇 MODERNIZED PROJECT LINK BUTTON START */}
                    <a
                      href={Project.link}
                      onMouseEnter={handleEnter}
                      onMouseLeave={handleExit}
                      rel="noopener noreferrer"
                      className="relative flex justify-center items-center group-hover:bg-darkGray/5 group-hover:dark:bg-white/5 rounded-xl w-10 h-10 overflow-hidden active:scale-95 transition-all group-hover:translate-x-4 duration-500"
                    >
                      <span className="flex flex-col justify-center items-center gap-2 w-full h-24 rotate-45 transition-all -translate-x-[19px] translate-y-5 hover:-translate-y-3 hover:translate-x-3 duration-300">
                        <NavigationArrow
                          className="mb-[1px] h-1/2 rotate-45"
                          size={'65%'}
                          weight="duotone"
                        />

                        <span className="h-1/2">Go!</span>
                      </span>
                    </a>
                  ...
                  <div className="flex flex-wrap items-center gap-1">
                    {Project.tech.map(item => {
                      return (
                        <span
                          key={item}
                          className="bg-darkGray/5 dark:bg-white/5 px-1.5 py-0.5 rounded text-[13px] smartphone:text-[8px] group-hover:scale-[102%] transition-all group-hover:translate-x-3 duration-[800ms] delay-75"
                        >
                          {item}
                        </span>
                      );
